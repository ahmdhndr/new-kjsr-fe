...
import { Button as ButtonTailwind } from "@/components/ui/button";
...
const ImageUploadPreview: React.FC<ImageUploadPreviewProps> = ({
  ...
  const previewUrl = fileItem.url || URL.createObjectURL(fileItem.file); // kalau udah sukses pakai url dari server, kalau belum pakai blob preview
  const token = TokenManager.getToken();
  const { mutateDeleteFile, isPendingDeleteFile } = useMediaMutation(token!);
  const handleRemove = async () => {
    const filePath = fileItem.url!.replace(`${env.NEXT_PUBLIC_MEDIA_URL}`, "");

    mutateDeleteFile({ filePath }, { onSuccess: () => onRemove() });
  };
