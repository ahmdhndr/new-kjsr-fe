...
import useListArticleReview from "../_hooks/use-list-review-articles";

export default function ArticleReviewTable({ token }: { token: string }) {
  const {
    articleData,
    currentPage,
    currentLimit,
  ...
  const columns: ColumnDef<Article>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Tooltip>
            <TooltipTrigger>
              <div className="line-clamp-1 w-full max-w-[400px] text-ellipsis">
                {article.title}
              </div>
            </TooltipTrigger>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button asChild>
            <Link href={`/dashboard/admin/review-articles/${article.id}`}>
              Detail
            </Link>
          </Button>
        );
