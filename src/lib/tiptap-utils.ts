...
export const handleImageUpload = async (
  file: File,
  mutateUploadFileAsync: Uploader,
  onProgress?: (event: { progress: number }) => void,
  abortSignal?: AbortSignal
): Promise<string> => {
  // Perform initial validation and pre-flight cancellation check
  ...
  try {
    ...
    // execute the actual upload by calling the passed mutate function
    const uploaded = mutateUploadFileAsync({
      file,
      folder: "images",
    });

    // Check for the cancellation again after the upload complete
    if (abortSignal?.aborted) {
      // Note: The file is already uploaded, but we stop the UI from proceeding.
      // You might want to trigger a delete request here in a real-world scenario.
      throw new DOMException("Upload cancelled by user.", "AbortError");
    }
    ...
    return (await uploaded).fullUrl;
  } catch (error) {
    // On error, reset progress and re-throw to be handled by the caller
    onProgress?.({ progress: 0 });
    console.error("Upload failed:", error);
    // Re-throw the original error for the calling component to catch
    throw error;
  }
};
